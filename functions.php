 ...
 */
if (!function_exists('light_blog_setup')) :
	...
	function light_blog_setup() {
		...
		add_theme_support('custom-background', apply_filters('light_blog_custom_background_args', array(
			'default-color' => 'ccc',
			'default-image' => '',
		)));
...
endif; // light_blog_setup
add_action('after_setup_theme', 'light_blog_setup');
 ...
 */
function light_blog_content_width() {
	$GLOBALS['content_width'] = apply_filters('light_blog_content_width', 800);
}

add_action('after_setup_theme', 'light_blog_content_width', 0);

/**
 * $content_width global variable adjustment as per layout option.
 */
function light_blog_dynamic_content_width() {
	global $post;
	...
	if ($light_blog_layout_meta == 'default_layout') {
		if (is_page()) {
			if ($light_blog_default_page_layout == 'no_sidebar_full_width') {
				$content_width = 1170; /* pixels */
			} else {
				$content_width = 800; /* pixels */
			}
		} elseif (is_single()) {
			if ($light_blog_default_post_layout == 'no_sidebar_full_width') {
				$content_width = 1170; /* pixels */
			} else {
				$content_width = 800; /* pixels */
			}
...
add_action('template_redirect', 'light_blog_dynamic_content_width');
 ...
 */
function light_blog_widgets_init() {
	...
	register_widget('light_blog_Tabbed_Widget');
	register_widget('light_blog_Random_Posts_Widget');
}

add_action('widgets_init', 'light_blog_widgets_init');
 ...
 */
if (!function_exists('light_blog_fonts_url')) {

	// Using google font
	// creating the function for adding the google font url
	function light_blog_fonts_url() {
		$fonts_url = '';
		$fonts = array();
...
function light_blog_scripts() {
	...
	if (get_theme_mod('light_blog_breaking_news', 0) == 1) {
	...
	if (get_theme_mod('light_blog_sticky_menu', 0) == 1) {
	...
	if ((get_theme_mod('light_blog_featured_image_popup', 0) == 1) && has_post_format('image') && has_post_thumbnail()) {
	...
	if (get_theme_mod('light_blog_sticky_sidebar_content', 0) == 1) {
...
add_action('wp_enqueue_scripts', 'light_blog_scripts');

/**
 * Enqueue scripts and styles in the customizer
 */
function light_blog_customizer_scripts() {
	// adding the function to load the minified version if SCRIPT_DEFUG is disable
	$suffix = ( defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ) ? '' : '.min';
...
add_action('customize_controls_enqueue_scripts', 'light_blog_customizer_scripts');
