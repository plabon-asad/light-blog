 ...
 * @return array
 */
function light_blog_body_classes($classes) {
...
add_filter('body_class', 'light_blog_body_classes');

/*
 * Display the date in the header
 */
if (!function_exists('light_blog_date_display')) :

	function light_blog_date_display() {
		?>
		<div class="date-in-header">
			<?php
			if (get_theme_mod('light_blog_date_display_type', 'theme_default') == 'theme_default') {
				echo date_i18n('l, F j, Y');
			} elseif (get_theme_mod('light_blog_date_display_type', 'theme_default') == 'wordpress_date_setting') {
 ...
 */
if (!function_exists('light_blog_breaking_news')) :

	function light_blog_breaking_news() {
 ...
 */
if (!function_exists('light_blog_social_menu')) :

	function light_blog_social_menu() {
 ...
 */
if (!function_exists('light_blog_header_text_logo')) :

	function light_blog_header_text_logo() {
		?>
 ...
 */
if (!function_exists('light_blog_category_color')) :

	function light_blog_category_color($wp_category_id) {
		$args = array(
			'orderby' => 'id',
			'hide_empty' => 0
		);
		...
		foreach ($category as $category_list) {
			$color = get_theme_mod('light_blog_category_color_' . $wp_category_id);
			return $color;
		}
	}
 ...
 */
if (!function_exists('light_blog_colored_category')) :

	function light_blog_colored_category() {
		global $post;
		...
		if ($categories) {
			foreach ($categories as $category) {
				$color_code = light_blog_category_color(get_cat_id($category->cat_name));
				if (!empty($color_code)) {
					$output .= '<a href="' . get_category_link($category->term_id) . '" style="background:' . light_blog_category_color(get_cat_id($category->cat_name)) . '" rel="category tag">' . $category->cat_name . '</a>' . $separator;
				} else {
 ...
 */
if (!function_exists('light_blog_colored_category_title')) :

	function light_blog_colored_category_title($title) {
		$color_value = light_blog_category_color(get_cat_id($title));
		$color_border_value = light_blog_category_color(get_cat_id($title));
...
function light_blog_category_title_function($category_title) {
	add_filter('single_cat_title', 'light_blog_colored_category_title');
}

add_action('light_blog_category_title', 'light_blog_category_title_function');

/*
 * Random Post in header
 */
if (!function_exists('light_blog_random_post')) :

	function light_blog_random_post() {
 ...
 */
add_filter('wp_nav_menu_items', 'light_blog_menu_filter', 10, 2);

if (!function_exists('light_blog_menu_filter')) :

	function light_blog_menu_filter($items, $args) {
		// adding custom added ul
		if (((get_theme_mod('light_blog_random_post_in_menu', 0) == 1) || (get_theme_mod('light_blog_search_icon_in_menu', 0) == 1 )) && $args->theme_location == 'primary') {
			$items .= '</ul><ul class="nav navbar-nav navbar-right">';
		}

		// adding the function to call up the random post
		if ((get_theme_mod('light_blog_random_post_in_menu', 0) == 1) && $args->theme_location == 'primary') {
			$items .= light_blog_random_post();
		}

		// showing the random post icon
		if ((get_theme_mod('light_blog_search_icon_in_menu', 0) == 1) && $args->theme_location == 'primary') {
...
add_action('light_blog_footer_copyright', 'light_blog_footer_copyright', 10);
/**
 * function to show the footer info, copyright information
 */
if (!function_exists('light_blog_footer_copyright')) :

	function light_blog_footer_copyright() {
 ...
 */
function light_blog_excerpt_length($length) {
	return 40;
}

add_filter('excerpt_length', 'light_blog_excerpt_length');

/**
 * Controlling the excerpt string
 */
function light_blog_excerpt_string($more) {
	return '&hellip;';
}

add_filter('excerpt_more', 'light_blog_excerpt_string');

/*
 * Display the related posts
 */
if (!function_exists('light_blog_related_posts_function')) :

	function light_blog_related_posts_function() {
		global $post;
		...
		if (get_theme_mod('light_blog_related_posts', 'categories') == 'categories') {
			$cats = wp_get_post_categories($post->ID, array('fields' => 'ids'));
			$args['category__in'] = $cats;
		}

		// Related by tags
		if (get_theme_mod('light_blog_related_posts', 'categories') == 'tags') {
 ...
 * posts pagnation
 */
if (!function_exists('light_blog_posts_pagination')) :

	function light_blog_posts_pagination() {
...
add_filter('body_class', 'light_blog_body_class');
 ...
 */
function light_blog_body_class($classes) {

	// custom layout options for posts and pages
	global $post;

	if ($post) {
		$light_blog_layout_meta = get_post_meta($post->ID, 'light_blog_page_layout', true);
	}

	if (is_home()) {
		$queried_id = get_option('page_for_posts');
		$light_blog_layout_meta = get_post_meta($queried_id, 'light_blog_page_layout', true);
	}

	if (empty($light_blog_layout_meta) || is_archive() || is_search() || is_404()) {
		$light_blog_layout_meta = 'default_layout';
	}

	$light_blog_default_layout = get_theme_mod('light_blog_default_layout', 'right_sidebar');
	$light_blog_default_page_layout = get_theme_mod('light_blog_default_page_layout', 'right_sidebar');
	$light_blog_default_post_layout = get_theme_mod('light_blog_default_single_posts_layout', 'right_sidebar');

	if ($light_blog_layout_meta == 'default_layout') {
		if (is_page()) {
			if ($light_blog_default_page_layout == 'right_sidebar') {
				$classes[] = 'right-sidebar';
			} elseif ($light_blog_default_page_layout == 'left_sidebar') {
				$classes[] = 'left-sidebar';
			} elseif ($light_blog_default_page_layout == 'no_sidebar_full_width') {
				$classes[] = 'no-sidebar-full-width';
			} elseif ($light_blog_default_page_layout == 'no_sidebar_content_centered') {
				$classes[] = 'no-sidebar-content-centered';
			}
		} elseif (is_single()) {
			if ($light_blog_default_post_layout == 'right_sidebar') {
				$classes[] = 'right-sidebar';
			} elseif ($light_blog_default_post_layout == 'left_sidebar') {
				$classes[] = 'left-sidebar';
			} elseif ($light_blog_default_post_layout == 'no_sidebar_full_width') {
				$classes[] = 'no-sidebar-full-width';
			} elseif ($light_blog_default_post_layout == 'no_sidebar_content_centered') {
				$classes[] = 'no-sidebar-content-centered';
			}
		} elseif ($light_blog_default_layout == 'right_sidebar') {
			$classes[] = 'right-sidebar';
		} elseif ($light_blog_default_layout == 'left_sidebar') {
			$classes[] = 'left-sidebar';
		} elseif ($light_blog_default_layout == 'no_sidebar_full_width') {
			$classes[] = 'no-sidebar-full-width';
		} elseif ($light_blog_default_layout == 'no_sidebar_content_centered') {
			$classes[] = 'no-sidebar-content-centered';
		}
	} elseif ($light_blog_layout_meta == 'right_sidebar') {
		$classes[] = 'right-sidebar';
	} elseif ($light_blog_layout_meta == 'left_sidebar') {
		$classes[] = 'left-sidebar';
	} elseif ($light_blog_layout_meta == 'no_sidebar_full_width') {
		$classes[] = 'no-sidebar-full-width';
	} elseif ($light_blog_layout_meta == 'no_sidebar_content_centered') {
		$classes[] = 'no-sidebar-content-centered';
	}

	// custom layout option for site
	if (get_theme_mod('light_blog_site_layout', 'wide_layout') == 'wide_layout') {
		$classes[] = 'wide';
	} elseif (get_theme_mod('light_blog_site_layout', 'wide_layout') == 'boxed_layout') {
		$classes[] = 'boxed';
	}
 ...
 */
if (!function_exists('light_blog_sidebar_select')) :

	function light_blog_sidebar_select() {
		global $post;
 ...
 */
if (!function_exists('light_blog_entry_meta_custom')) :

	function light_blog_entry_meta_custom() {
 ...
 */
if (!function_exists('light_blog_author_bio_links')) :

	function light_blog_author_bio_links() {
...
if (!function_exists('light_blog_link_post_format')) :

	function light_blog_link_post_format() {
...
if (!function_exists('light_blog_audio_video_post_format')) :

	function light_blog_audio_video_post_format() {
...
if (!function_exists('light_blog_status_post_format_first_paragraph')) :

	function light_blog_status_post_format_first_paragraph() {
...
if (!function_exists('light_blog_status_post_format_avatar_image')) :

	function light_blog_status_post_format_avatar_image() {
...
if (!function_exists('light_blog_quote_post_format_blockquote')) :

	function light_blog_quote_post_format_blockquote() {
...
if (!function_exists('light_blog_paginate_links')) :

	function light_blog_paginate_links() {
		?>
		<div class="post-nav">
			<?php
			global $wp_query;
...
function light_blog_paginate_links_display() {
	echo light_blog_paginate_links();
}

/**
 * Migrate any existing theme CSS codes added in Customize Options to the core option added in WordPress 4.7
 */
function light_blog_custom_css_migrate() {
	if (function_exists('wp_update_custom_css_post')) {
		$custom_css = get_theme_mod('light_blog_custom_css');
		if ($custom_css) {
			$core_css = wp_get_custom_css(); // Preserve any CSS already added to the core option.
			$return = wp_update_custom_css_post($core_css . $custom_css);
			if (!is_wp_error($return)) {
				// Remove the old theme_mod, so that the CSS is stored in only one place moving forward.
				remove_theme_mod('light_blog_custom_css');
			}
		}
	}
}

add_action('after_setup_theme', 'light_blog_custom_css_migrate');
...
add_action('woocommerce_before_main_content', 'light_blog_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'light_blog_wrapper_end', 10);

function light_blog_wrapper_start() {
	echo '<div id="primary" class="content-area col-md-8"><main id="main" class="site-main" role="main">';
}

function light_blog_wrapper_end() {
	echo '</main></div><!-- #primary -->';
	light_blog_sidebar_select();
}
