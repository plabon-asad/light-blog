 ...
 */
add_action('add_meta_boxes', 'light_blog_custom_meta_boxes');

/**
 * Adding the Custom Meta Box
 */
function light_blog_custom_meta_boxes() {
	// Adding the layout meta box for single page
	add_meta_box('page-layout', esc_html__('Select Layout', 'creative-blog'), 'light_blog_page_layout', 'page', 'side', 'default');
	// Adding the layout meta box for single post page
	add_meta_box('page-layout', esc_html__('Select Layout', 'creative-blog'), 'light_blog_page_layout', 'post', 'side', 'default');
}

/**
 * Adding the sidebar display of the meta option in the editor
 */
global $light_blog_page_layout;
$light_blog_page_layout = array(
	'default-layout' => array(
		'id' => 'light_blog_page_layout',
 ...
 */
function light_blog_page_layout() {
	global $light_blog_page_layout, $post;

	// Use nonce for verification
	wp_nonce_field(basename(__FILE__), 'custom_meta_box_nonce');

	foreach ($light_blog_page_layout as $field) {
		$light_blog_layout_meta = get_post_meta($post->ID, $field['id'], true);
		if (empty($light_blog_layout_meta)) {
			$light_blog_layout_meta = 'default_layout';
		}
		?>
		<input class="post-format" id="<?php echo esc_attr($field['value']); ?>" type="radio" name="<?php echo esc_attr($field['id']); ?>" value="<?php echo esc_attr($field['value']); ?>" <?php checked($field['value'], $light_blog_layout_meta); ?>/>
 ...
 */
if (!function_exists('light_blog_save_custom_meta_data')) :

	function light_blog_save_custom_meta_data($post_id) {
		global $light_blog_page_layout, $post;
...
add_action('pre_post_update', 'light_blog_save_custom_meta_data');
