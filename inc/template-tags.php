 ...
 */
if (!function_exists('light_blog_posted_on')) :

	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function light_blog_posted_on() {
 ...
 * @return bool
 */
function light_blog_categorized_blog() {
	if (false === ( $all_the_cool_cats = get_transient('light_blog_categories') )) {
		...
		set_transient('light_blog_categories', $all_the_cool_cats);
	}

	if ($all_the_cool_cats > 1) {
		// This blog has more than 1 category so light_blog_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so light_blog_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in light_blog_categorized_blog.
 */
function light_blog_category_transient_flusher() {
	if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient('light_blog_categories');
}

add_action('edit_category', 'light_blog_category_transient_flusher');
add_action('save_post', 'light_blog_category_transient_flusher');
